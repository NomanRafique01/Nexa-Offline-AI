...
// Built-in Python snippets surfaced in the Snippets dropdown.
...
// Default split: editor 58%, terminal 42%
...
const CodeRunnerModal = ({
  ...
  // Keeps onRunRef in sync so handleRun always calls the latest onRun prop
  ...
  // handleInputSubmit - handles user submitting an inline input() value.
  const handleInputSubmit = (value) => {
    if (value.trim() === "") {
      setTerminalLines((prev) => [...prev, { type: "system", text: "Input cannot be empty." }]);
      return false;
    }
    setTerminalLines((prev) => [...prev, { type: "input-value", text: value }]);
    if (inputRequest?.resolve) {
  ...
  // Incrementally append new stdout/stderr/plot lines to the terminal.
  // Uses refs to avoid re-appending lines that were already rendered.
  useEffect(() => {
    const newLines = [];
  ...
  // Close the snippet dropdown when the user clicks outside its wrapper.
  useEffect(() => {
    const h = (e) => {
  ...
  // handleRun - orchestrates a Python execution and collects inputs.
    ...
    try {
      ...
      if (totalInputs > 0) {
        ...
        while ((match = regex.exec(code)) !== null) {
          prompts.push(match[2] || 'Enter value: ');
        }
        while (prompts.length < totalInputs) prompts.push('Enter value: ');

        const collectedInputs = [];
        for (const prompt of prompts) {
          const value = await new Promise((resolve) => {
            setInputRequest({ prompt, resolve });
          });
          collectedInputs.push(value);
        }
        onRunRef.current?.(collectedInputs, runId, totalInputs);
        return;
      }
      onRunRef.current?.([], runId, 0);
    } finally {
